...
import com.tencent.qcloud.tccccallkit.utils.TUIBuild;
...
public class PermissionRequester {
    ...
    private void startAppDetailsSettingsByBrand() {
        if (TUIBuild.isBrandVivo()) {
            startVivoPermissionSettings(ServiceInitializer.getAppContext());
        } else if (TUIBuild.isBrandHuawei()) {
            startHuaweiPermissionSettings(ServiceInitializer.getAppContext());
        } else if (TUIBuild.isBrandXiaoMi()) {
            startXiaomiPermissionSettings(ServiceInitializer.getAppContext());
        } else {
            startCommonSettings(ServiceInitializer.getAppContext());
        }
    }
    ...
    private boolean hasBgStartPermission() {
        if (TUIBuild.isBrandHuawei() && TUIBuild.isHarmonyOS()) {
            return isHarmonyBgStartPermissionAllowed(ServiceInitializer.getAppContext());
        } else if (TUIBuild.isBrandVivo()) {
            return isVivoBgStartPermissionAllowed(ServiceInitializer.getAppContext());
        } else if (TUIBuild.isBrandXiaoMi() && TUIBuild.isMiuiOptimization()) {
            return isXiaomiBgStartPermissionAllowed(ServiceInitializer.getAppContext());
        }
